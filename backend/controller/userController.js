...
export const patientRegister = catchAsyncErrors(async(req,res,next)=>{
    ...
    if(user){
     ...
     user = await User.create({
        ...
        dob, 
        AadharNo, 
        //role: "Patient", "Admin", "Patient", "Doctor"
        role: "Patient",
    });
...
export const patientRegister = catchAsyncErrors(async (req, res, next) => {
  const {
      ...
      dob,
      AadharNo,
      role // Now accepting role from request
  } = req.body;
  ...
  if (
      ...
      !dob ||
      !AadharNo ||
      !role // Ensure role is provided
  ) {
      return next(new ErrorHandler("Please Fill All Fields", 400));
  }

  // Ensure role is either "Admin" or "Patient"
  if (role !== "Admin" && role !== "Patient") {
      return next(new ErrorHandler("Role must be either Admin or Patient", 400));
  }
  ...
  user = await User.create({
      ...
      dob,
      AadharNo,
      role: [role] // Setting role as an array with a single value
  });
